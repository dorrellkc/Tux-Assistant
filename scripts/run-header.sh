...
get_install_packages() {
    ...
    case "$pm" in
        ...
        dnf)
            ...
            ;;
        zypper)
            echo "python3 python3-gobject python3-gobject-Gdk typelib-1_0-Gtk-4_0 typelib-1_0-Adw-1 libadwaita-1-0 gstreamer-plugins-base gstreamer-plugins-good python3-pip python3-numpy python3-pydub"
            ;;
        *)
            echo ""
            ;;
...
install_dependencies() {
    ...
    case "$pm" in
        pacman)
            ...
            # Install librosa via pip (Arch is permissive)
            pip install --quiet librosa 2>/dev/null || true
            ;;
        apt)
            pkexec bash -c "apt update && apt install -y $packages"
            # Install librosa via pip
            pip3 install --break-system-packages --quiet librosa 2>/dev/null || true
            ;;
        dnf)
            pkexec dnf install -y $packages
            # Install librosa via pip
            pip3 install --quiet librosa 2>/dev/null || true
            ;;
        zypper)
