...
'''
    ...
    def create_main_page(self) -> Adw.NavigationPage:
        ...
        # System Fetch (fastfetch) button
        fetch_group = Adw.PreferencesGroup()
        fetch_group.set_title("Quick Tools")
        
        fastfetch_row = Adw.ActionRow()
        fastfetch_row.set_title("System Fetch")
        fastfetch_row.set_subtitle("Show detailed system info in terminal")
        fastfetch_row.set_activatable(True)
        fastfetch_row.add_prefix(Gtk.Image.new_from_icon_name("utilities-system-monitor-symbolic"))
        fastfetch_row.add_suffix(Gtk.Image.new_from_icon_name("utilities-terminal-symbolic"))
        fastfetch_row.connect("activated", self._on_fastfetch_clicked)
        fetch_group.add(fastfetch_row)
        
        self.modules_content_box.append(fetch_group)
...
read'''
    ...
    def _on_fastfetch_clicked(self, row):
        ...
        # Find available terminal and run fastfetch
        terminals = [
            ('gnome-terminal', ['gnome-terminal', '--', 'bash', '-c', 'fastfetch; echo; read -p "Press Enter to close..."']),
            ('konsole', ['konsole', '-e', 'bash', '-c', 'fastfetch; echo; read -p "Press Enter to close..."']),
            ('xfce4-terminal', ['xfce4-terminal', '-e', 'bash -c "fastfetch; echo; read -p \\"Press Enter to close...\\""']),
            ('kitty', ['kitty', 'bash', '-c', 'fastfetch; echo; read -p "Press Enter to close..."']),
            ('alacritty', ['alacritty', '-e', 'bash', '-c', 'fastfetch; echo; read -p "Press Enter to close..."']),
            ('tilix', ['tilix', '-e', 'bash -c "fastfetch; echo; read -p \\"Press Enter to close...\\""']),
            ('xterm', ['xterm', '-e', 'bash', '-c', 'fastfetch; echo; read -p "Press Enter to close..."']),
        ]
